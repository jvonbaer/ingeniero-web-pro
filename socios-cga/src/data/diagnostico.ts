 ...
 * cuenta de quien atiende— y la aplicación sólo avisaría del último que
 * fallara, en medio de otra tarea. Esto los prueba de a uno y dice cuál falta.
 *
...
const TITULOS: Record<(typeof ORDEN)[number], string> = {
  ...
  tablas: "Las seis tablas están creadas",
  rls: "Los datos no se leen sin iniciar sesión",
  sesion: "La cuenta del club inicia sesión",
...
export async function revisarInstalacion({
  ...
  if (expuestas.length > 0) {
    ...
    hechas.set("rls", {
      ...
      titulo: TITULOS.rls,
      estado: "falla",
      detalle: `Sin sesión ya se alcanzan a leer filas de: ${detalle}. Los RUT, teléfonos y direcciones de socios y menores están a la vista de cualquiera que abra el sitio.`,
      remedio:
        "Falta la parte de seguridad del esquema. Vuelva a correr supabase/schema.sql completo: " +
        "activa RLS en las seis tablas y deja las políticas sólo para usuarios con sesión.",
    });
    ...
    hechas.set("rls", {
      ...
      remedio: "Escriba el correo y la clave de una cuenta del club y vuelva a revisar.",
    });
  }

  if (!correo || !clave) {
    const motivo = "Falta el correo y la clave de una cuenta del club.";
...
async function probarTablas(
  ...
  return {
    prueba: {
      ...
      titulo,
      estado: "ok",
      detalle: "personas, vinculos, planes, inscripciones, pagos y avisos responden.",
    },
    expuestas,
  };
}
...
async function probarLectura(sb: Cliente): Promise<Prueba> {
  ...
  return { id: "lectura", titulo, estado: "ok", detalle: "Las seis tablas se dejan consultar." };
}

/**
 * Escribe un plan de prueba y lo borra. Es la única forma de saber que las
 * políticas de escritura están puestas: leer funciona con políticas a medias.
 *
 * Se usa `planes` y no `personas` a propósito: un plan no arrastra datos
 * personales, así que si el borrado fallara, lo que queda en la base es una
 * fila inocua y reconocible en vez de una ficha con nombre y RUT inventados.
 */
async function probarEscritura(sb: Cliente): Promise<Prueba> {
  ...
  const codigo = `PLAN DE PRUEBA ${marca}`;

  const alta = await sb.from("planes").insert({
    id,
    nombre: codigo,
    tipo: "actividad",
    rama: "club",
  ...
  const baja = await sb.from("planes").delete().eq("id", id);
  if (baja.error) {
    return {
      id: "escritura",
      titulo,
      estado: "aviso",
      detalle: `Se pudo guardar, pero no borrar el plan de prueba: ${baja.error.message}`,
      remedio: `Borre a mano la fila «${codigo}» en Supabase → Table Editor → planes.`,
    };
  }
  ...
  return {
    ...
    titulo,
    estado: "ok",
    detalle: "Se guardó un plan de prueba y se borró sin dejar rastro.",
  };
}
